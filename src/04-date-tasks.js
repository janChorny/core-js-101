...
function timeSpanToString(startDate, endDate) {
  ...
  if (msec < 10) msec = `00${date.getUTCMilliseconds()}`;
  if (msec >= 10 && msec < 100) msec = `0${date.getUTCMilliseconds()}`;
  if (msec >= 100) msec = `${date.getUTCMilliseconds()}`;
  const sec = date.getUTCSeconds() < 10 ? `0${date.getUTCSeconds()}` : `${date.getUTCSeconds()}`;
